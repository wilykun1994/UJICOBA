...
case 'listsurah':
                   if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fakeTroli})
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/quran?apikey=HENTONGERS`)
...
case 'hentai4everyone':
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fakeTroli})
                sticWait(from)
                hasil = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=HENTONGERS`)
                but = [
...
case 'nsfw_avatar':
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fakeTroli})
                sticWait(from)
                hasil = await getBuffer(`https://api.lolhuman.xyz/api/random2/${command}?apikey=HENTONGERS`)
                but = [
                { buttonId: `${prefix + command}`, buttonText: { displayText: '➡️ SELANJUTNNYA KAK ➡️ ' }, type: 1 }
                       ]
                sendButImage(from, '🥳 ℕ𝕚𝕙 𝕜𝕒𝕜 𝕦𝕕𝕒𝕙 𝕛𝕒𝕕𝕚 🥳', `🤖 Hentongers-Botz 🤖`, hasil, but)
